...
- **Next (gated):** (a) commit `silver_statement_text.py`; (b) `transform/gold_statement_extraction.py` (thin Spark wrapper over `statement_analyst.build_statement_rows`) → run on `gold_test` reading `silver_test.statement_text`; (c) **D-711 labeled sample — needs the operator** to read ~6–10 statements (Wolf + recent deals) for ground truth, mirroring D-706; (d) tier-2 accuracy gate → PROD on approval. **FU-703** (rotate exposed Consumer Secret) still open.
