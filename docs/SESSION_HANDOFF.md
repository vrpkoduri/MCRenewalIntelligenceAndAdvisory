...
- **Follow-ups open:** **FU-702** (upstream ops — populate `Application_Submission__c` + tag doc-type at intake; deferred), **FU-703** (rotate briefly-exposed Consumer Secret), **FU-704** (revenue-outlier guard — agent emits per-deposit line items → deterministic outlier drop; revenue is advisory/soft so deferred), **FU-705** (deterministic MCA-funder registry — agent extracts counterparty, a curated list [seed from observed funders + reuse `mca_funders`] confirms real advances regardless of amount variability, excludes leases/loans, unknowns→REVIEW; build only if the labeled sample shows v2 misses the ±1 bar).
