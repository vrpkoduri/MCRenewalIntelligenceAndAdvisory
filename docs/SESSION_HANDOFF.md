...
- **Prompt v2 shipped (2026-07-06, `4e1f895`):** tightened the MCA-position definition (exclude equipment leases, bank/SBA term loans, vendor/SaaS/hosting bills, cards, one-offs). gold_test before/after on the 10-deal sample: **A1a 1→0** (hosting), **Tom Snell 4→2** (leases), **Flener 5→2** (SBA/bank loans); clean cases held (All Point 4, Wolf 3, 5 Star 2, abstains 0). Residuals: **Hjm 3→2** (Coast, a real but variable-amount MCA, got excluded — possible mild over-correction, *within the ±1 gate tolerance*); **Wolf revenue** still counts a $605k one-off (LLM won't catch it → FU-704).
- **Next:** **D-711 labeled sample — needs the operator**: true count of genuine cash advances per deal (+ rough monthly revenue for Wolf/5 Star/Hjm) for the 10 sampled → encode `STATEMENT_LABELS` → re-run → gate scores v2 → PROD on approval. Then dashboard surfacing (advisory).
- **Reuse check (C-027, 2026-07-09):** investigated `mca_funders` — it has the routing team's own statement extraction (`merchant_submission_positions` + `_monthly_metrics` + `merchant_submissions`), BUT **0 overlap with MRI's funded book** (app-id `OM_…`≠`a0o…`; tax_id/fein join = 0; only 191 submissions, the new-deal funnel). **So no reuse shortcut — the Statement Analyst build STANDS** (it's the only source for the funded book's statements). `silver.funders` = 12 *partner* funders → partial seed for FU-705.
- **Follow-ups open:** **FU-702** (upstream ops; deferred), **FU-703** (rotate secret), **FU-704** (revenue-outlier guard; deferred), **FU-705** (combined MCA-position ID — registry allowlist [seed: `silver.funders` 12 partners + routing `lender_name` vocab + observed funders] ∪ recurrence pattern, minus lease/loan/vendor denylist, unknowns→REVIEW; **planned per user "do both"**), **FU-706** (align with routing team on a shared funder registry + common statement-extraction service; their `days_with_nsf` also serves FU-301).
- **✅ PROMOTED TO PROD `gold` (C-030, 2026-07-14):** OCR → PROD `silver.statement_text` (324 files) then extraction → PROD `gold.merchant_extraction` (237 rows: 14 applied / 223 review) + `statement_extraction_audit` (79) + 237 `agent_extraction` events (v2). **Verified in PROD:** `_current` holds both streams — Data Steward `default_subtype` applied **5** (Starr intact) + statement positions applied 5; advisory-only (no rung change). **S7 Phase-2 is DONE in PROD.** Remaining are non-blocking: dashboard surfacing, FU-703 (rotate secret — deferred by operator), FU-704/705/706/707, and the PR to `main`.
- **D-711 positions gate PASSED (C-028, 2026-07-13):** operator-confirmed labels encoded → tier-2 recon on `gold_test` (v2, 79 deals, 237 rows) → **labeled_accuracy 1.0 (10/10), `failures: []`**; mechanical gates all green; ±1 tolerance absorbed A1a's 0-vs-1 wobble; revenue excluded (FU-704). Permanent PROD gate now.
- **ONLY remaining gated step → PROD promotion (needs explicit approval + `allow_prod=True`, Rule 5):** `build_gold_statement_extraction(schema="gold", allow_prod=True)` — writes the statement extractions (+ `statement_extraction_audit`) + `agent_extraction` events for the 79 covered deals to PROD `gold.merchant_extraction`. **Advisory-only (C-026 #1) — does NOT change any rung.** Then (separate) dashboard surfacing + FU-705 registry + FU-704 revenue.
