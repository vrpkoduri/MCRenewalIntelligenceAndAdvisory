...
- **Next:** (a) **D-711 labeled sample — needs the operator**: ground truth (positions/weekly-debit/est-revenue) for ~6–10 deals incl. Wolf; I'll surface the OCR text to make it fast → arm the gate → re-run → PROD on approval. (b) Surface statement burden/positions on the dashboard (advisory, `burden_source=statement`). **FU-702** (ops, deferred), **FU-703** (rotate secret) open.
