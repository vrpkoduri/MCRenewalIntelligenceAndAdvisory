...
- **Next (gated):** (a) **silver OCR** of the Volume PDFs → `silver.statement_text` (D-708: `ai_parse_document`/pdfplumber); (b) `transform/gold_statement_extraction.py` (thin Spark wrapper over `statement_analyst.build_statement_rows`) → `gold_test`; (c) **D-711 labeled sample — needs the operator** to read ~6–10 statements (Wolf + recent deals) for ground truth (positions/burden/revenue), mirroring D-706; (d) tier-2 accuracy gate → PROD on approval. **FU-703** (rotate exposed Consumer Secret) still open.
