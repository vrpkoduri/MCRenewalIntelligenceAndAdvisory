...
- **Pending gate step:** operator confirmed v2's 10-deal counts "look good" → encode as `STATEMENT_LABELS` (positions), score the D-711 positions gate on `gold_test` (revenue deferred, FU-704), then build FU-705 → re-run → PROD on approval.
