...
### Sprint 8 (advisory layer + first-class compliance gate, §2.3/§2.4/§5.9)
Added: `test_compliance` (14) + `test_advisory` (14). **Compliance** covers the advice-vs-specific-offer classifier (concrete-term ⇒ specific-offer; action-no-terms ⇒ advice; neither ⇒ factual-summary), the state disclosure lookup (CA/NY/UT/VA case-insensitive; unknown/absent ⇒ NONE; required only for a specific offer in a regulated state), and the HARD gate's four dispositions — PASS grounded honest advice; BLOCK ungrounded; BLOCK a suppressed/WAIT offer pitched as specific (the §2.3 failure); BLOCK a specific offer missing a required disclosure (PASS once the disclosure block is attached). **Advisory** covers the fact pack (present-only, never fabricated; pct dual-form 0.20/20%; money cent/dollar rounding so $20,879.9999→$20,880 is grounded), the grounding validator (grounded text OK; invented $50,000 rejected; ISO dates), the Structure Advisor (Wolf → wait-and-pay-down, $20,880 double-dip **reused from S5 not recomputed**, cannot un-suppress), the compose→ground→gate orchestration (honest-wait advice PASS+APPLIED; ungrounded → REJECTED; suppressed double-dip pitched → BLOCKED+REVIEW; specific offer needs an NY disclosure block; batch), and map/no-surface/no-spine-column invariants. The agents ARTICULATE only — an agent can neither invent a number (grounding rejects it) nor self-certify compliance (the gate decides).
**Result (2026-07-18): 28 new (14 compliance + 14 advisory) → 352 passed, 0 failed (tier-1).**

Tier-2 (`recon_advisory.py`) + the D-807 labeled quality sample (grounding 100% + gate correctness + honesty) + the `transform/gold_advisory.py` Spark driver are **gated** (Rule 5) — `gold_test` first, PROD on approval; **delivery is a separate later gated step** (S9+). The Spark schema builder `merchant_advisory_schema()` is exercised in tier-2 (no pyspark in the tier-1 env; tier-1 asserts the column list against the field map, as the other gold tables do).
