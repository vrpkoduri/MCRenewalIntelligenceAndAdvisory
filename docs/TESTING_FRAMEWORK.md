...
### Sprint 7 (agentic extraction — deterministic tools, §5.9)
Added: `test_agents` (12). Covers the Data Steward default-subtype mapper (synonyms → enum; junk → unknown, never guess), the confidence gate (low-conf → unknown+review+do-not-fund; concrete+confident → applied+route), the grounding contract (ungrounded → rejected & not-applicable, model_version + known type required, review_status), the lifecycle gate honoring a resolved sub-type (the agent EXTRACTS / the gate ROUTES; unresolved unchanged), and map/no-surface/event-type/threshold invariants.
**Result (2026-06-05): 12 new → 297 passed, 0 failed (tier-1).**
Pending (gated — LLM cost OK, cloud gated): the Data Steward **LLM agent** (Databricks Foundation Model over the 6 `closed_default` merchants' Notes → label+confidence → the deterministic gate → `gold.merchant_extraction`) + an S3 re-run on the resolved sub-types, on `gold_test`; validate vs Starr; quantify the `unknown`-pile resolution. Phase 2 (Statement Analyst) deferred (D-702 statement-ingestion spike).
