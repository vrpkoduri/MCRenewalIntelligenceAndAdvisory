...
| `agents.positions` (S7 Phase 2, C-025/C-026) | `src/common/agents/positions.py` | Statement Analyst deterministic counter: `normalize_to_weekly` (Daily/Weekly REUSE `clock.rollup.weekly_debit` — Rule 3; +Biweekly/Monthly via `Thresholds`), `concurrent_position_count` (distinct funder streams; excludes Morgan Cash's own by default), `total_weekly_debit`, `est_weekly_revenue` (deposits ÷ period-weeks; None when unknown — never a fabricated 0), `statement_is_fresh` (C-026 #2 freshness window), `summarize_statement`. Pure; built + tier-1-tested offline. |
| `agents.statement_analyst` (S7 Phase 2, C-026) | `src/common/agents/statement_analyst.py` | The fuzzy half (Spark-free, injected `predict_fn`): grounded prompt (operating-revenue-only, #3), tolerant/defensive `parse_response`, `classify_statement`, and `build_statement_rows` — the pure agent→count→ground orchestration emitting 3 grounded extractions (concurrent_positions / weekly_debit / est_weekly_revenue). Bakes in **C-026**: #1 advisory-only (never feeds the rung waterfall — recorded + surfaced only), #2 freshness (stale → REVIEW, `as_of` in source_ref), #3 revenue confidence haircut. The real driver `transform/gold_statement_extraction.py` (gated) supplies the Databricks chat client + OCR'd `silver.statement_text`. |
| `ingestion.statement_fetch` (S7 Phase 2, D-707) | `src/ingestion/statement_fetch.py` | Bank-statement binary fetch SF Files→UC Volume. Pure helpers (tier-1): `is_statement` (Document_Type__c='Bank Statement' ∪ title heuristic), `location_to_deal_map` (`FirstPublishLocationId`→funded deal via opp/`Application_Submission__c`), `covered_statement`, `statement_file_row`. Driver: `sf_token` (Client Credentials), `enumerate_statements` (paginated SOQL), `download_version_data`, `fetch_statements` → writes PDFs to Volume `mca_mri.bronze.statements_raw` + `bronze.statement_files`. **RUN 2026-06-09: 324 PDFs / 167.9 MB / 79 deals landed.** PII: Volume UC-governed; only derived aggregates leave the agent (D-712). |
