...
Same pattern; stage `recon_extraction.py` + `run_tier2_extraction`. Extra `base_parameters`: `endpoint` (default `databricks-claude-sonnet-4-5` — verify provisioned via `databricks serving-endpoints list`), `rerun_s3` (`true` re-runs S3 to route on the resolved sub-types). Calls a **Foundation Model via the `databricks-sdk` serving client** (serverless has the SDK, NOT `mlflow`); LLM spend is bounded to the `closed_default` deals (~6). The recon enforces a **D-706 accuracy gate** (`DEFAULT_SUBTYPE_LABELS` ground truth, `ACCURACY_BAR=0.80`) — HARD-fails on regressions. **Prompt v2** (`model_version …/v2`): an explicit "defaulted" overrides a "clawback" mention. Run 2026-06-05 (C-023→C-024): v1 = 4/6 (caught the clawback mis-read) → **v2 = 6/6, `failures: []`** (5 true_default→distressed-exit, 1 unknown→review). **PROD `gold` promotion now requires only explicit approval + `allow_prod=True`** (Rule 5).

### S8 Advisory layer (LLM — `run_tier2_advisory`)
Same pattern; stage `recon_advisory.py` + `run_tier2_advisory`. `transform/gold_advisory.build_gold_advisory(spark, schema, run_date, allow_prod, sample_merchant_ids, predict_fn)` composes a **grounded, compliance-gated advisory per active merchant** (Advisory Composer + Structure Advisor) and writes point-in-time `gold.merchant_advisory` (+`_current`) + `advisory_composed`/`compliance_checked` events. It READS the gold `_current` views (clock/rung/predictions + the most-recent active position for the double-dip factor/balance) + `gold.merchants.governing_state`; it **never recomputes the spine**, and it **does NOT send** (stored, not delivered). Reuses the S7 Foundation-Model caller (`databricks-sdk` serving client). Widgets: `schema` (default `gold_test`), `allow_prod`, `run_date`, `endpoint`, `sample_merchant_ids` (comma-separated — **sample-first**: run the D-807 labeled set cheaply before the whole book). The recon HARD-asserts gate integrity (**no BLOCKED advisory is ever marked deliverable**; every APPLIED advisory is PASS; grounding is 100% by construction — an invented number is REJECTED before storage) + the **D-807 quality gate** (`recon_advisory.ADVISORY_LABELS`, `ACCURACY_BAR=0.80`) once the operator fills it.
- **v1 signal coverage (honest):** the SPECIFIC-OFFER path (naming a concrete advance amount) depends on `gold.merchant_offers` (**FU-501**, gated); until it lands the driver degrades to grounded ADVICE + structure guidance (`offer_type=None`). The driver reads `merchant_offers_current` automatically once it exists.
- **Before a PROD run (Rule 5 + the two S8 gates):** (1) **D-805** — counsel review of the disclosure state-list (`constants.DISCLOSURE_RULES`, seeded CA/NY/UT/VA) before merchant-facing output; (2) **D-807** — fill `recon_advisory.ADVISORY_LABELS` (four merchants central; Wolf → wait-and-pay-down) and pass the sample as `sample_merchant_ids`. `gold_test` first; **PROD `gold` requires `schema=gold` AND `allow_prod=True` + explicit approval.** No delivery in S8 (S9+).
