...
DEALS_MAP: tuple[FieldSpec, ...] = (
    ...
    FieldSpec("deal_type", "enum", "Opportunity.Type", Verdict.HAVE, "100% set; trust as renewal flag. Real values: New Business / Renewal / Stack / Add-On (Buyout valid but absent) — repeat advances = all non-New (FU-601)"),
...
DEAL_TABLE_MAP: tuple[FieldSpec, ...] = (
    ...
    FieldSpec("deal_type", "enum", "silver.deals.deal_type", Verdict.HAVE, "New Business / Renewal / Stack / Add-On (Buyout valid but absent) — trusted (CLAUDE.md 2.5); repeat advances = all non-New (FU-601)"),
    FieldSpec("is_renewal_of", "string", "derive:renewal chain (D-103)", Verdict.DERIVE, "deal_id of immediately-prior same-merchant deal by funded_date for any repeat advance (Renewal/Buyout/Stack/Add-On, FU-601); null for New Business / first deal"),
