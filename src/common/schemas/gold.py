...
def merchant_predictions_schema():
    """StructType for mca_mri.gold.merchant_predictions (S6 Prediction, point-in-time, D-604)."""
    from pyspark.sql.types import StructField, StructType

    fields = [StructField(fs.silver_col, _spark_type(fs.dtype), True) for fs in MERCHANT_PREDICTIONS_MAP]
    fields += [
...
def merchant_advisory_schema():
    """StructType for mca_mri.gold.merchant_advisory (S8 advisory layer, point-in-time, D-804)."""
    from pyspark.sql.types import StructField, StructType

    return StructType(
        [StructField(fs.silver_col, _spark_type(fs.dtype), True) for fs in MERCHANT_ADVISORY_MAP]
    )
